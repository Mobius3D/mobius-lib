...
const config = {
  ...
  getSqsQueue(type, opts) {
    if (!(type in config.getSqsQueueFunctions)) {
      throw new Error(`Configuration error: getSqsQueue("${type}") invoked for unknown type`);
    }

    return config.getSqsQueueFunctions[type](opts);
  }
};
...
function loadConfig(dirPath, secretsPath) {
  if (!config.isLoaded) {
    ...
    if (!utils.isEmpty(config.vals.bucketKey)) {
      config.getBucketKeyFunctions = { ...config.vals.bucketKey };
    }

    // TODO: ... or these ...
    if (!utils.isEmpty(config.vals.sqsQueue)) {
      config.getSqsQueueFunctions = { ...config.vals.sqsQueue };
    }
