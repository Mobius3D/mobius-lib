...
function init(opts) {
  ...
  if (opts.log) {
    if (opts.log.warn) log.warn = opts.log.warn;
    if (opts.log.debug) log.debug = opts.log.debug;
  }

  if (opts.Stats && opts.Stats.update) {
    Stats.update = opts.Stats.update;
  }
